...
def create_new_account():
    ...
    def create_account():
        ...
        if (len(user_name)==0 or len(pass_word)==0 or len(confirm_password)==0 or len(user_mail)==0 ):
            ...
            messagebox.showinfo('USER NAME','Your username should contain atleast 5 characters')
        elif(len(pass_word)<6):
            messagebox.showinfo('PASSWORD','Your password should contain atleast 6 characters')
        elif (pass_word != confirm_password):
            messagebox.showinfo('confirm password','passwords not matched, please re-enter password')
